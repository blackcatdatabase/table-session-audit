 ...
 */
final class SessionAuditAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private SessionAuditRepository $sessionAuditRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'session_audit' => $this->sessionAuditRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\SessionAudit\ModuleException("Record {$idStr} not found for locking.");
            }

...
use BlackCat\Database\Packages\SessionAudit\Dto\SessionAuditDto as Dto;
use BlackCat\Database\Packages\SessionAudit\Mapper\SessionAuditDtoMapper as RowMapper;
...
final class SessionAuditRepository implements RepoContract, KeysetRepoContract
{
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\SessionAudit\Definitions::class; }
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\SessionAudit\Definitions::table());
    ...
    */
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\SessionAudit\Dto\SessionAuditDto|null */
    public function getById(int $id, bool $asDto = false): array|\BlackCat\Database\Packages\SessionAudit\Dto\SessionAuditDto|null {
        $row = $this->getByUnique([ 'id' => $id ]);
        if (!$asDto || !$row) return $row;
        return \BlackCat\Database\Packages\SessionAudit\Mapper\SessionAuditDtoMapper::fromRow($row);
    }
